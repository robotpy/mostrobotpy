...
class Intake(Subsystem):
    def __init__(self) -> None:
        super().__init__()
        self.motor = wpilib.PWMSparkMax(IntakeConstants.K_MOTOR_PORT)

        # Double solenoid connected to two channels of a PCM with the default CAN ID
        self.pistons = wpilib.DoubleSolenoid(
            0,
            wpilib.PneumaticsModuleType.CTRE_PCM,
            IntakeConstants.K_SOLENOID_PORTS[0],
            IntakeConstants.K_SOLENOID_PORTS[1],
        )
    ...
    def intake_command(self) -> Command:
        ...
        return (
            ...
            .and_then(self.run(lambda: self.motor.set(1.0)))
            .with_name("Intake")
        )
