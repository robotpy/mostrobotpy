...
K_JOYSTICK_PORT = 0
K_HIGH_GOAL_POSITION = wpimath.units.feet_to_meters(3)
K_LOW_GOAL_POSITION = wpimath.units.feet_to_meters(0)

K_CARRIAGE_MASS = 4.5
# kilograms

# A 1.5in diameter drum has a radius of 0.75in, or 0.019in.
K_DRUM_RADIUS = 1.5 / 2.0 * 25.4 / 1000.0

# Reduction between motors and encoder, as output over input. If the elevator spins slower than
# the motors, this number should be greater than one.
K_ELEVATOR_GEARING = 6.0
...
class MyRobot(wpilib.TimedRobot):
    ...
    def __init__(self) -> None:
        ...
        self.encoder = wpilib.Encoder(K_ENCODER_A_CHANNEL, K_ENCODER_B_CHANNEL)

        self.motor = wpilib.PWMSparkMax(K_MOTOR_PORT)

        # A joystick to read the trigger from.
        self.joystick = wpilib.Joystick(K_JOYSTICK_PORT)

        # Circumference = pi * d, so distance per click = pi * d / counts
        self.encoder.set_distance_per_pulse(math.tau * K_DRUM_RADIUS / 4096)
    ...
    def teleop_periodic(self) -> None:
        ...
        if self.joystick.get_trigger():
            ...
            goal = wpimath.TrapezoidProfile.State(K_HIGH_GOAL_POSITION, 0.0)

        else:
            # Otherwise, we go to the low goal
            goal = wpimath.TrapezoidProfile.State(K_LOW_GOAL_POSITION, 0.0)
        ...
        self.loop.set_next_r(
            [self.last_profiled_reference.position, self.last_profiled_reference.velocity]
        )
