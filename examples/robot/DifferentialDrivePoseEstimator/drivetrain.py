...
class Drivetrain:
    ...
    K_MAX_VELOCITY = 3.0  # meters per second
    K_MAX_ANGULAR_VELOCITY = math.tau  # one rotation per second

    K_TRACKWIDTH = 0.381 * 2  # meters
    K_WHEEL_RADIUS = 0.0508  # meters
    ...
    def __init__(self, camera_to_object_topic: ntcore.DoubleArrayTopic) -> None:
        ...
        self.kinematics = wpimath.DifferentialDriveKinematics(self.K_TRACKWIDTH)
        ...
        layout = robotpy_apriltag.AprilTagFieldLayout.load_field(
            robotpy_apriltag.AprilTagField.K_2024_CRESCENDO
        )
        ...
        self.left_encoder.set_distance_per_pulse(
            math.tau * self.K_WHEEL_RADIUS / self.K_ENCODER_RESOLUTION
        )
    ...
    def update_odometry(self) -> None:
        ...
        vision_measurement_3_d = self.object_to_robot_pose(
            self.object_in_field, self.robot_to_camera, self.camera_to_object_entry
        )

        # Convert robot pose from Pose3d to Pose2d needed to apply vision measurements.
        vision_measurement_2_d = vision_measurement_3_d.to_pose_2_d()
    ...
    def simulation_periodic(self) -> None:
        ...
        self.drivetrain_simulator.set_inputs(
            self.left_leader.get_throttle() * wpilib.RobotController.get_input_voltage(),
            self.right_leader.get_throttle() * wpilib.RobotController.get_input_voltage(),
        )
        self.drivetrain_simulator.update(0.02)

        self.left_encoder_sim.set_distance(self.drivetrain_simulator.get_left_position())
    ...
    def periodic(self) -> None:
        ...
        self.field_approximation.set_robot_pose(self.pose_estimator.get_estimated_position())
