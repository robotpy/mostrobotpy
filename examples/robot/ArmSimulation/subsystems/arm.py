...
class Arm:
    def __init__(self) -> None:
        ...
        self.arm_kp = Constants.K_DEFAULT_ARM_KP
        self.arm_setpoint_degrees = Constants.K_DEFAULT_ARM_SETPOINT_DEGREES

        # The arm gearbox represents a gearbox containing two Vex 775pro motors.
        self.arm_gearbox = wpimath.DCMotor.vex_775_pro(2)
        ...
        self.arm_sim = wpilib.simulation.SingleJointedArmSim(
            self.arm_gearbox,
            Constants.K_ARM_REDUCTION,
            wpilib.simulation.SingleJointedArmSim.estimate_moi(
                Constants.K_ARM_LENGTH, Constants.K_ARM_MASS
            ),
            Constants.K_ARM_LENGTH,
            Constants.K_MIN_ANGLE_RADS,
            Constants.K_MAX_ANGLE_RADS,
            True,
            0,
            [Constants.K_ARM_ENCODER_DIST_PER_PULSE, 0.0],
        )
        ...
        self.encoder.set_distance_per_pulse(Constants.K_ARM_ENCODER_DIST_PER_PULSE)
        ...
        wpilib.Preferences.init_double(
            Constants.K_ARM_POSITION_KEY, self.arm_setpoint_degrees
        )
        wpilib.Preferences.init_double(Constants.K_ARM_P_KEY, self.arm_kp)
    ...
    def load_preferences(self) -> None:
        ...
        if self.arm_kp != wpilib.Preferences.get_double(Constants.K_ARM_P_KEY, self.arm_kp):
            self.arm_kp = wpilib.Preferences.get_double(Constants.K_ARM_P_KEY, self.arm_kp)
