...
class DriveSubsystem(commands2.Subsystem):
    def __init__(self) -> None:
        ...
        # The robot's drive
        self.drive = wpilib.DifferentialDrive(self.left_leader.set, self.right_leader.set)

        self.profile = wpimath.TrapezoidProfile(
            wpimath.TrapezoidProfile.Constraints(
                constants.DriveConstants.K_MAX_VELOCITY,
                constants.DriveConstants.K_MAX_ACCELERATION,
            )
        )
    ...
    def set_drive_states(
        ...
        left_feedforward = self.feedforward.calculate(
            ...
            (next_left.velocity - current_left.velocity) / constants.DriveConstants.K_DT,
        )
        right_feedforward = self.feedforward.calculate(
            current_right.velocity,
            (next_right.velocity - current_right.velocity) / constants.DriveConstants.K_DT,
        )
        self.left_leader.set_set_point(
            examplesmartmotorcontroller.ExampleSmartMotorController.PIDMode.K_POSITION,
            current_left.position,
            left_feedforward / battery_voltage,
        )

        self.right_leader.set_set_point(
            examplesmartmotorcontroller.ExampleSmartMotorController.PIDMode.K_POSITION,
    ...
    def profiled_drive_distance(self, distance: float) -> commands2.Command:
        ...
        def on_execute():
            ...
            next_setpoint = self.profile.calculate(
                current_time + constants.DriveConstants.K_DT,
                wpimath.TrapezoidProfile.State(),
                wpimath.TrapezoidProfile.State(distance, 0),
            )
        ...
        def on_end(interrupted: bool):
            self.left_leader.set(0)
            self.right_leader.set(0)
    ...
    def dynamic_profiled_drive_distance(self, distance: float) -> commands2.Command:
        ...
        def on_execute():
            ...
            current_left = self.profile.calculate(
                ...
                wpimath.TrapezoidProfile.State(self._initial_left_distance + distance, 0),
            )
