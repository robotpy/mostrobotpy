...
class Shooter(Subsystem):
    ...
    def __init__(self) -> None:
        ...
        self.shooter_motor = PWMSparkMax(ShooterConstants.K_SHOOTER_MOTOR_PORT)

        # The motor on the feeder wheels.
        self.feeder_motor = PWMSparkMax(ShooterConstants.K_FEEDER_MOTOR_PORT)

        # The shooter wheel encoder
        self.shooter_encoder = Encoder(
            ShooterConstants.K_ENCODER_PORTS[0],
            ShooterConstants.K_ENCODER_PORTS[1],
            ShooterConstants.K_ENCODER_REVERSED,
        )
        ...
        # calculated by SysId
        self.shooter_feedback = wpimath.PIDController(ShooterConstants.K_P, 0, 0)
        ...
        self.shooter_feedforward = wpimath.SimpleMotorFeedforwardRadians(
            ShooterConstants.K_S,
            ShooterConstants.K_V / wpimath.units.rotations_to_radians(1),
            ShooterConstants.K_A / wpimath.units.rotations_to_radians(1),
        )

        # Sets the distance per pulse for the encoders
        self.shooter_encoder.set_distance_per_pulse(
            ShooterConstants.K_ENCODER_DISTANCE_PER_PULSE
        )
    ...
    def run_shooter(self, shooter_velocity: Callable[[], float]) -> Command:
        ...
        def _run_shooter() -> None:
            ...
            target_velocity_radians = wpimath.units.rotations_to_radians(target_velocity)
            self.shooter_motor.set_voltage(
            ...
            self.feeder_motor.set_throttle(ShooterConstants.K_FEEDER_VELOCITY)
