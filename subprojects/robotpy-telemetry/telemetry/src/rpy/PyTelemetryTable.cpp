...
bool PyTelemetryTable::IsWpiStruct(py::handle value) {
  return py::hasattr(py::type::of(value), "WPIStruct");
}

bool PyTelemetryTable::IsWpiStructType(py::handle value) {
  return PyType_Check(value.ptr()) && py::hasattr(value, "WPIStruct");
}

bool PyTelemetryTable::IsBuiltinType(py::handle value, const char* name) {
  return value.is(py::module_::import("builtins").attr(name));
}

bool PyTelemetryTable::IsNoElementType(
    const std::optional<ElementType>& elementType) {
  return !elementType;
}

std::optional<py::object> PyTelemetryTable::GetOptionalAttr(py::handle value,
                                                            const char* name) {
...
void PyTelemetryTable::ValidateStructSequenceType(const py::sequence& value,
                                                  const py::type& type) {
