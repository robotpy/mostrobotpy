...
def get_test_trajectory(config: TrajectoryConfig) -> Trajectory:
    ...
    return TrajectoryGenerator.generate_trajectory(side_start, vector, cross_scale, config)
...
def test_elliptical_region_constraint():
    ...
    for point in trajectory.states():
        if ellipse.contains(point.pose.translation()):
            assert abs(point.velocity_fps) < max_velocity + 0.05
        elif abs(point.velocity_fps) >= max_velocity + 0.05:
...
def test_rectangular_region_constraint():
    max_velocity = 2
    rectangle = Rectangle2d(Translation2d.from_feet(1, 1), Translation2d.from_feet(5, 27))
