...
def test_relative_to():
    initial = Pose2d(x=0, y=0, rotation=Rotation2d.from_degrees(45))
    final = Pose2d(x=5, y=5, rotation=Rotation2d.from_degrees(45.0))
...
def test_rotate_around():
    initial = Pose2d(x=5, y=0, rotation=Rotation2d.from_degrees(0))
...
def test_equality():
    a = Pose2d(x=0, y=5, rotation=Rotation2d.from_degrees(43))
    b = Pose2d(x=0, y=5, rotation=Rotation2d.from_degrees(43))
    assert a == b


def test_inequality():
    a = Pose2d(x=0, y=5, rotation=Rotation2d.from_degrees(43))
    b = Pose2d(x=0, y=feet_to_meters(5), rotation=Rotation2d.from_degrees(43))
    assert a != b


def test_minus():
    initial = Pose2d(x=0, y=0, rotation=Rotation2d.from_degrees(45))
    final = Pose2d(x=5, y=5, rotation=Rotation2d.from_degrees(45))
...
def test_nearest():
    ...
    translation = Translation2d(distance=1.0, angle=Rotation2d.from_degrees(0))

    pose_a = Pose2d(translation, Rotation2d.from_degrees(0))
    pose_b = Pose2d(translation, Rotation2d.from_degrees(30))
    pose_c = Pose2d(translation, Rotation2d.from_degrees(120))
    pose_d = Pose2d(translation, Rotation2d.from_degrees(90))
    pose_e = Pose2d(translation, Rotation2d.from_degrees(-180))

    assert Pose2d(x=0, y=0, rotation=Rotation2d.from_degrees(360)).nearest(
        [pose_a, pose_b, pose_d]
    ).rotation().degrees() == pytest.approx(pose_a.rotation().degrees())
    assert Pose2d(x=0, y=0, rotation=Rotation2d.from_degrees(-335)).nearest(
        [pose_b, pose_c, pose_d]
    ).rotation().degrees() == pytest.approx(pose_b.rotation().degrees())
    assert Pose2d(x=0, y=0, rotation=Rotation2d.from_degrees(-120)).nearest(
        [pose_b, pose_c, pose_d]
    ).rotation().degrees() == pytest.approx(pose_c.rotation().degrees())
    assert Pose2d(x=0, y=0, rotation=Rotation2d.from_degrees(85)).nearest(
        [pose_a, pose_c, pose_d]
    ).rotation().degrees() == pytest.approx(pose_d.rotation().degrees())
    assert Pose2d(x=0, y=0, rotation=Rotation2d.from_degrees(170)).nearest(
        [pose_a, pose_d, pose_e]
    ).rotation().degrees() == pytest.approx(pose_e.rotation().degrees())


def test_to_matrix():
    before = Pose2d(x=1, y=2, rotation=Rotation2d.from_degrees(20))
    after = Pose2d.from_matrix(before.to_matrix())
