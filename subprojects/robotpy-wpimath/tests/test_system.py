...
def test_rk_4_exponential():
    """Test that integrating dx/dt = eˣ works"""
    y0 = np.array([[0.0]])

    y1 = wpimath.rk_4(lambda x: np.array([[math.exp(x[0, 0])]]), y0, 0.1)

    assert math.isclose(y1[0, 0], math.exp(0.1) - math.exp(0.0), abs_tol=1e-3)


def test_rk_4_exponential_with_u():
...
def test_numerical_jacobian():
    ...
    new_a = wpimath.numerical_jacobian(ax_fn, np.zeros((4, 1)))
...
def test_numerical_jacobian_x_u_square():
    ...
    x0 = np.zeros((4, 1))
    u0 = np.zeros((2, 1))
    new_a = wpimath.numerical_jacobian_x(axbu_fn, x0, u0)
    new_b = wpimath.numerical_jacobian_u(axbu_fn, x0, u0)
    np.testing.assert_allclose(new_a, a, rtol=1e-6, atol=1e-5)
...
def test_numerical_jacobian_x_u_rectangular():
    ...
    x0 = np.zeros((4, 1))
    u0 = np.zeros((2, 1))
    new_c = wpimath.numerical_jacobian_x(cxdu_fn, x0, u0)
    new_d = wpimath.numerical_jacobian_u(cxdu_fn, x0, u0)
...
def test_numerical_jacobian_x_passes_extra_args():
    ...
    new_a = wpimath.numerical_jacobian_x(axbu_fn, x0, u0, 2.5, -3.0)
...
def test_numerical_jacobian_u_passes_extra_args():
    ...
    new_b = wpimath.numerical_jacobian_u(axbu_fn, x0, u0, 4.0, 0.25)
