...
def test_initialize():
    odometry = DifferentialDriveOdometry3d(
        gyro_angle=Rotation3d.from_degrees(0, 0, 90),
        left_distance=0,
        right_distance=0,
...
def test_encoder_distances():
    odometry = DifferentialDriveOdometry3d(
        gyro_angle=Rotation3d.from_degrees(0, 0, 45), left_distance=0, right_distance=0
    )

    pose = odometry.update(Rotation3d.from_degrees(0, 0, 135), 0, 5 * math.pi)

    assert pose.x == pytest.approx(5.0, abs=1e-9)
    assert pose.y == pytest.approx(5.0, abs=1e-9)
    assert pose.z == pytest.approx(0.0, abs=1e-9)
    assert pose.rotation().to_rotation_2_d().degrees() == pytest.approx(90.0, abs=1e-9)
