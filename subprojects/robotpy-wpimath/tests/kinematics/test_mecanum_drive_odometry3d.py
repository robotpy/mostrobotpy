...
def test_initialize(odometry_3_d_test):
    odometry = MecanumDriveOdometry3d(
        odometry_3_d_test.kinematics,
        gyro_angle=Rotation3d(),
        wheel_positions=odometry_3_d_test.zero,
    ...
    assert pose.rotation().to_rotation_2_d().degrees() == pytest.approx(45, abs=1e-9)


def test_multiple_consecutive_updates(odometry_3_d_test):
    wheel_deltas = MecanumDriveWheelPositions(
        front_left=3.536, front_right=3.536, rear_left=3.536, rear_right=3.536
    )

    odometry_3_d_test.odometry.reset_position(Rotation3d(), wheel_deltas, Pose3d())
    odometry_3_d_test.odometry.update(Rotation3d(), wheel_deltas)
    second_pose = odometry_3_d_test.odometry.update(Rotation3d(), wheel_deltas)
    ...
    assert second_pose.rotation().to_rotation_2_d().radians() == pytest.approx(
        0.0, abs=0.01
    )


def test_two_iterations(odometry_3_d_test):
    odometry_3_d_test.odometry.reset_position(Rotation3d(), odometry_3_d_test.zero, Pose3d())
    wheel_deltas = MecanumDriveWheelPositions(
        front_left=0.3536, front_right=0.3536, rear_left=0.3536, rear_right=0.3536
    )

    odometry_3_d_test.odometry.update(Rotation3d(), MecanumDriveWheelPositions())
    pose = odometry_3_d_test.odometry.update(Rotation3d(), wheel_deltas)
    ...
    assert pose.rotation().to_rotation_2_d().radians() == pytest.approx(0.0, abs=0.01)
...
def test_90_degree_turn(odometry_3_d_test):
    ...
    odometry_3_d_test.odometry.update(Rotation3d(), MecanumDriveWheelPositions())
    pose = odometry_3_d_test.odometry.update(
    ...
    assert pose.rotation().to_rotation_2_d().degrees() == pytest.approx(90.0, abs=0.01)


def test_gyro_angle_reset(odometry_3_d_test):
    odometry_3_d_test.odometry.reset_position(
        Rotation3d.from_degrees(0, 0, 90), odometry_3_d_test.zero, Pose3d()
    )
...
def test_accuracy_facing_trajectory():
    ...
    assert error_sum / (trajectory.total_time() / dt) < 0.07
