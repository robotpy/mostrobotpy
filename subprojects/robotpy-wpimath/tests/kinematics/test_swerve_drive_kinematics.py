...
def test_straight_line_inverse_kinematics(kinematics_test):
    ...
    assert fl.velocity == pytest.approx(5.0, abs=k_epsilon)
    assert fr.velocity == pytest.approx(5.0, abs=k_epsilon)
    assert bl.velocity == pytest.approx(5.0, abs=k_epsilon)
    assert br.velocity == pytest.approx(5.0, abs=k_epsilon)

    assert fl.angle.radians() == pytest.approx(0.0, abs=k_epsilon)
    assert fr.angle.radians() == pytest.approx(0.0, abs=k_epsilon)
    assert bl.angle.radians() == pytest.approx(0.0, abs=k_epsilon)
    assert br.angle.radians() == pytest.approx(0.0, abs=k_epsilon)
...
def test_straight_line_forward_kinematics(kinematics_test):
    ...
    assert chassis_velocities.vx == pytest.approx(5.0, abs=k_epsilon)
    assert chassis_velocities.vy == pytest.approx(0.0, abs=k_epsilon)
    assert chassis_velocities.omega == pytest.approx(0.0, abs=k_epsilon)
...
def test_straight_line_forward_kinematics_with_deltas(kinematics_test):
    ...
    twist = kinematics_test.m_kinematics.to_twist_2_d((delta, delta, delta, delta))

    assert twist.dx == pytest.approx(5.0, abs=k_epsilon)
    assert twist.dy == pytest.approx(0.0, abs=k_epsilon)
    assert twist.dtheta == pytest.approx(0.0, abs=k_epsilon)
...
def test_straight_strafe_inverse_kinematics(kinematics_test):
    ...
    assert fl.angle.degrees() == pytest.approx(90.0, abs=k_epsilon)
    assert fr.angle.degrees() == pytest.approx(90.0, abs=k_epsilon)
    assert bl.angle.degrees() == pytest.approx(90.0, abs=k_epsilon)
    assert br.angle.degrees() == pytest.approx(90.0, abs=k_epsilon)
...
def test_straight_strafe_forward_kinematics(kinematics_test):
    ...
    assert chassis_velocities.vx == pytest.approx(0.0, abs=k_epsilon)
    assert chassis_velocities.vy == pytest.approx(5.0, abs=k_epsilon)
    assert chassis_velocities.omega == pytest.approx(0.0, abs=k_epsilon)
...
def test_straight_strafe_forward_kinematics_with_deltas(kinematics_test):
    ...
    assert twist.dx == pytest.approx(0.0, abs=k_epsilon)
    assert twist.dy == pytest.approx(5.0, abs=k_epsilon)
    assert twist.dtheta == pytest.approx(0.0, abs=k_epsilon)
...
def test_turn_in_place_inverse_kinematics(kinematics_test):
    ...
    assert fl.velocity == pytest.approx(106.63, abs=k_epsilon)
    assert fr.velocity == pytest.approx(106.63, abs=k_epsilon)
    assert bl.velocity == pytest.approx(106.63, abs=k_epsilon)
    assert br.velocity == pytest.approx(106.63, abs=k_epsilon)

    assert fl.angle.degrees() == pytest.approx(135.0, abs=k_epsilon)
    assert fr.angle.degrees() == pytest.approx(45.0, abs=k_epsilon)
    assert bl.angle.degrees() == pytest.approx(-135.0, abs=k_epsilon)
    assert br.angle.degrees() == pytest.approx(-45.0, abs=k_epsilon)


def test_conserve_wheel_angle(kinematics_test):
    velocities = ChassisVelocities(vx=0, vy=0, omega=2 * math.pi)
    kinematics_test.m_kinematics.to_swerve_module_velocities(velocities)
    states = kinematics_test.m_kinematics.to_swerve_module_velocities(ChassisVelocities())

    fl, fr, bl, br = states

    assert fl.velocity == pytest.approx(0.0, abs=k_epsilon)
    assert fr.velocity == pytest.approx(0.0, abs=k_epsilon)
    assert bl.velocity == pytest.approx(0.0, abs=k_epsilon)
    assert br.velocity == pytest.approx(0.0, abs=k_epsilon)
...
def test_reset_wheel_angle(kinematics_test):
    ...
    kinematics_test.m_kinematics.reset_headings((fl_angle, fr_angle, bl_angle, br_angle))
    states = kinematics_test.m_kinematics.to_swerve_module_velocities(ChassisVelocities())

    fl_mod, fr_mod, bl_mod, br_mod = states

    assert fl_mod.velocity == pytest.approx(0.0, abs=k_epsilon)
    assert fr_mod.velocity == pytest.approx(0.0, abs=k_epsilon)
    assert bl_mod.velocity == pytest.approx(0.0, abs=k_epsilon)
    assert br_mod.velocity == pytest.approx(0.0, abs=k_epsilon)

    assert fl_mod.angle.degrees() == pytest.approx(0.0, abs=k_epsilon)
    assert fr_mod.angle.degrees() == pytest.approx(90.0, abs=k_epsilon)
    assert bl_mod.angle.degrees() == pytest.approx(180.0, abs=k_epsilon)
    assert br_mod.angle.degrees() == pytest.approx(-90.0, abs=k_epsilon)
...
def test_turn_in_place_forward_kinematics(kinematics_test):
    ...
    assert chassis_velocities.vx == pytest.approx(0.0, abs=k_epsilon)
    assert chassis_velocities.vy == pytest.approx(0.0, abs=k_epsilon)
    assert chassis_velocities.omega == pytest.approx(2 * math.pi, abs=k_epsilon)
...
def test_turn_in_place_forward_kinematics_with_deltas(kinematics_test):
    ...
    twist = kinematics_test.m_kinematics.to_twist_2_d((fl, fr, bl, br))

    assert twist.dx == pytest.approx(0.0, abs=k_epsilon)
    assert twist.dy == pytest.approx(0.0, abs=k_epsilon)
    assert twist.dtheta == pytest.approx(2 * math.pi, abs=k_epsilon)
...
def test_off_center_cor_rotation_inverse_kinematics(kinematics_test):
    ...
    assert fl.velocity == pytest.approx(0.0, abs=k_epsilon)
    assert fr.velocity == pytest.approx(150.796, abs=k_epsilon)
    assert bl.velocity == pytest.approx(150.796, abs=k_epsilon)
    assert br.velocity == pytest.approx(213.258, abs=k_epsilon)

    assert fl.angle.degrees() == pytest.approx(0.0, abs=k_epsilon)
    assert fr.angle.degrees() == pytest.approx(0.0, abs=k_epsilon)
    assert bl.angle.degrees() == pytest.approx(-90.0, abs=k_epsilon)
    assert br.angle.degrees() == pytest.approx(-45.0, abs=k_epsilon)
...
def test_off_center_cor_rotation_forward_kinematics(kinematics_test):
    ...
    assert chassis_velocities.vx == pytest.approx(75.398, abs=k_epsilon)
    assert chassis_velocities.vy == pytest.approx(-75.398, abs=k_epsilon)
    assert chassis_velocities.omega == pytest.approx(2 * math.pi, abs=k_epsilon)
...
def test_off_center_cor_rotation_forward_kinematics_with_deltas(kinematics_test):
    ...
    assert twist.dx == pytest.approx(75.398, abs=k_epsilon)
    assert twist.dy == pytest.approx(-75.398, abs=k_epsilon)
    assert twist.dtheta == pytest.approx(2 * math.pi, abs=k_epsilon)
...
def test_off_center_cor_rotation_and_translation_inverse_kinematics(kinematics_test):
    ...
    assert fl.velocity == pytest.approx(23.43, abs=k_epsilon)
    assert fr.velocity == pytest.approx(23.43, abs=k_epsilon)
    assert bl.velocity == pytest.approx(54.08, abs=k_epsilon)
    assert br.velocity == pytest.approx(54.08, abs=k_epsilon)

    assert fl.angle.degrees() == pytest.approx(-140.19, abs=k_epsilon)
    assert fr.angle.degrees() == pytest.approx(-39.81, abs=k_epsilon)
    assert bl.angle.degrees() == pytest.approx(-109.44, abs=k_epsilon)
    assert br.angle.degrees() == pytest.approx(-70.56, abs=k_epsilon)
...
def test_off_center_cor_rotation_and_translation_forward_kinematics(kinematics_test):
    ...
    assert chassis_velocities.vx == pytest.approx(0.0, abs=k_epsilon)
    assert chassis_velocities.vy == pytest.approx(-33.0, abs=k_epsilon)
    assert chassis_velocities.omega == pytest.approx(1.5, abs=k_epsilon)
...
def test_off_center_cor_rotation_and_translation_forward_kinematics_with_deltas(
    ...
    assert twist.dx == pytest.approx(0.0, abs=k_epsilon)
    assert twist.dy == pytest.approx(-33.0, abs=k_epsilon)
    assert twist.dtheta == pytest.approx(1.5, abs=k_epsilon)
...
def test_desaturate(kinematics_test):
    ...
    assert arr[0].velocity == pytest.approx(5.0 * k_factor, abs=k_epsilon)
    assert arr[1].velocity == pytest.approx(6.0 * k_factor, abs=k_epsilon)
    assert arr[2].velocity == pytest.approx(4.0 * k_factor, abs=k_epsilon)
    assert arr[3].velocity == pytest.approx(7.0 * k_factor, abs=k_epsilon)
...
def test_desaturate_negative_velocity(kinematics_test):
    ...
    assert arr[0].velocity == pytest.approx(0.5, abs=k_epsilon)
    assert arr[1].velocity == pytest.approx(0.5, abs=k_epsilon)
    assert arr[2].velocity == pytest.approx(-1.0, abs=k_epsilon)
    assert arr[3].velocity == pytest.approx(-1.0, abs=k_epsilon)
