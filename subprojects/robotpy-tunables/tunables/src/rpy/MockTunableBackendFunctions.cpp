...
py::object ReadTunableValue(
  ...
  if (auto value = wpi::tunables::detail::CastTunable<T, false>(info.tunable,
                                                                info.type)) {
    return ValueToPython(value->Get());
  }
  if (auto value = wpi::tunables::detail::CastTunable<T, true>(info.tunable,
                                                               info.type)) {
    return ValueToPython(value->Get(info.config->parent));
  }
...
py::bytes ReadStructValue(
    const wpi::tunables::TunableRegistry::TunableInfo& info) {
  if (auto value = wpi::tunables::detail::CastTunable<
          wpi::tunables::detail::TunableStructTag, false>(info.tunable,
                                                          info.type)) {
    std::vector<uint8_t> data(value->GetStructSize());
    value->PackStruct(data);
    return py::bytes{reinterpret_cast<const char*>(data.data()), data.size()};
  }
  if (auto value = wpi::tunables::detail::CastTunable<
          wpi::tunables::detail::TunableStructTag, true>(info.tunable,
                                                         info.type)) {
    std::vector<uint8_t> data(value->GetStructSize(info.config->parent));
    value->PackStruct(info.config->parent, data);
    return py::bytes{reinterpret_cast<const char*>(data.data()), data.size()};
  }
  throw py::type_error("tunable has unexpected type");
}
...
void SetRaw(wpi::tunables::MockTunableBackend& backend, std::string_view path,
            RawValue value) {
  auto data = ToRawVector(value);
  backend.SetRaw(path, std::span<const uint8_t>{data});
}

void SetBoolVector(wpi::tunables::MockTunableBackend& backend,
                   std::string_view path, py::typing::Iterable<bool> value) {
  backend.SetBoolVector(path, ToVector<bool>(value));
}

void SetInt32Vector(wpi::tunables::MockTunableBackend& backend,
                    std::string_view path,
                    py::typing::Iterable<int32_t> value) {
  auto data = ToVector<int32_t>(value);
  backend.SetInt32Vector(path, std::span<const int32_t>{data});
}

void SetInt64Vector(wpi::tunables::MockTunableBackend& backend,
                    std::string_view path,
                    py::typing::Iterable<int64_t> value) {
  auto data = ToVector<int64_t>(value);
  backend.SetInt64Vector(path, std::span<const int64_t>{data});
}

void SetFloatVector(wpi::tunables::MockTunableBackend& backend,
                    std::string_view path, py::typing::Iterable<float> value) {
  auto data = ToVector<float>(value);
  backend.SetFloatVector(path, std::span<const float>{data});
}

void SetDoubleVector(wpi::tunables::MockTunableBackend& backend,
                     std::string_view path,
                     py::typing::Iterable<double> value) {
  auto data = ToVector<double>(value);
  backend.SetDoubleVector(path, std::span<const double>{data});
}

void SetStringVector(wpi::tunables::MockTunableBackend& backend,
                     std::string_view path,
                     py::typing::Iterable<std::string> value) {
  auto data = ToVector<std::string>(value);
  backend.SetStringVector(path, std::span<const std::string>{data});
}

void SetStruct(wpi::tunables::MockTunableBackend& backend,
               std::string_view path, py::handle value) {
  WPyStructInfo info{py::type::of(value)};
  backend.SetStruct<WPyStruct, WPyStructInfo>(
...
void SetStructVector(wpi::tunables::MockTunableBackend& backend,
  ...
  backend.SetStructVector<WPyStruct, WPyStructInfo>(
      path, std::span<const WPyStruct>{data}, std::move(info));
}

std::optional<uint32_t> GetUid(const wpi::tunables::MockTunableBackend& backend,
                               std::string_view path) {
  return backend.GetUid(path);
}

py::object GetTunableValue(const wpi::tunables::MockTunableBackend& backend,
                           std::string_view path) {
  auto uid = backend.GetUid(path);
  if (!uid) {
