...
def test_timeout(scheduler: commands2.CommandScheduler):
    with ManualSimTime() as sim:
        command_1 = commands2.WaitCommand(1)
...
def test_and_then(scheduler: commands2.CommandScheduler):
    ...
    command_2 = commands2.InstantCommand(lambda: condition.set(True))
    scheduler.schedule(command_1.and_then(command_2))
...
def test_along_with(scheduler: commands2.CommandScheduler):
    ...
    command_1 = commands2.WaitUntilCommand(condition)
    command_2 = commands2.InstantCommand()

    group = command_1.along_with(command_2)
...
def test_race_with(scheduler: commands2.CommandScheduler):
    command_1 = commands2.WaitUntilCommand(lambda: False)
    command_2 = commands2.InstantCommand()

    group = command_1.race_with(command_2)
