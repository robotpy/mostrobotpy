...
def test_on_true(scheduler: commands2.CommandScheduler):
    ...
    command_1 = commands2.WaitUntilCommand(finished)

    button = InternalButton()
    button.set_pressed(False)
    button.on_true(command_1)
    scheduler.run()
    assert not command_1.is_scheduled()
    button.set_pressed(True)
    scheduler.run()
    assert command_1.is_scheduled()
    finished.set(True)
    scheduler.run()
    assert not command_1.is_scheduled()
...
def test_while_true_repeatedly(scheduler: commands2.CommandScheduler):
    ...
    command_1 = commands2.FunctionalCommand(
...
def test_while_true_lambda_run_command(scheduler: commands2.CommandScheduler):
    ...
    command_1 = commands2.RunCommand(counter.increment_and_get)
...
def test_while_true_once(scheduler: commands2.CommandScheduler):
    ...
    command_1 = commands2.StartEndCommand(
...
def test_toggle_on_true(scheduler: commands2.CommandScheduler):
    ...
    button = InternalButton()
    button.set_pressed(False)
    button.toggle_on_true(command_1)
    scheduler.run()
    assert start_counter == 0
    assert end_counter == 0
    button.set_pressed(True)
