...
class SingleCompositionTestBase:
    ...
    @pytest.mark.parametrize(
        ...
        [
            commands2.InterruptionBehavior.K_CANCEL_SELF,
            commands2.InterruptionBehavior.K_CANCEL_INCOMING,
        ],
    )
    ...
    def test_run_when_disabled(self, runs_when_disabled: bool):
        command = self.compose_single(
            commands2.WaitUntilCommand(lambda: False).ignoring_disable(runs_when_disabled)
        )
...
class MultiCompositionTestBase(SingleCompositionTestBase):
    ...
    @pytest.mark.parametrize(
        "expected,command_1,command_2,command_3",
        [
            pytest.param(
                commands2.InterruptionBehavior.K_CANCEL_SELF,
                commands2.WaitUntilCommand(lambda: False).with_interrupt_behavior(
    ...
    def test_interruptible(self, expected, command_1, command_2, command_3):
        command = self.compose(command_1, command_2, command_3)
    ...
    def test_run_when_disabled(self, expected, command_1, command_2, command_3):
        command = self.compose(command_1, command_2, command_3)
