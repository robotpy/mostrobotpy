...
class ConditionalCommand(Command):
    ...
    def get_interruption_behavior(self) -> InterruptionBehavior:
        if (
            self.on_true.get_interruption_behavior() == InterruptionBehavior.K_CANCEL_SELF
            or self.on_false.get_interruption_behavior()
            == InterruptionBehavior.K_CANCEL_SELF
        ):
            return InterruptionBehavior.K_CANCEL_SELF
        else:
            return InterruptionBehavior.K_CANCEL_INCOMING
