...
def test_1():
    ...
    estimator = robotpy_apriltag.AprilTagPoseEstimator(
        robotpy_apriltag.AprilTagPoseEstimator.Config(
            tag_size=0.2, fx=500, fy=500, cx=320, cy=240
        )
    )

    est = estimator.estimate_orthogonal_iteration(results[0], 50)
    assert est.pose_2 == Transform3d()
    pose = estimator.estimate(results[0])
    assert est.pose_1 == pose
...
def test_pose_rotated_x():
    ...
    est = estimator.estimate_orthogonal_iteration(results[0], 50)
    assert pytest.approx(est.pose_1.rotation().x, abs=0.1) == math.radians(45)
    assert pytest.approx(est.pose_1.rotation().y, abs=0.1) == math.radians(0)
    assert pytest.approx(est.pose_1.rotation().z, abs=0.1) == math.radians(0)
...
def test_pose_rotated_y():
    ...
    est = estimator.estimate_orthogonal_iteration(results[0], 50)
    assert pytest.approx(est.pose_1.rotation().x, abs=0.1) == math.radians(0)
    assert pytest.approx(est.pose_1.rotation().y, abs=0.1) == math.radians(45)
    assert pytest.approx(est.pose_1.rotation().z, abs=0.1) == math.radians(0)
